...
export async function setAllReg() {
  ...
  if (result.node) {
    if (!fs.existsSync(npmrcPath)) {
      console.log(chalk.cyan("未找到.npmrc文件,正在创建"));
      fs.writeFile(npmrcPath, "", (err) => {
        if (err) {
          console.log(err);
        } else {
          console.log(chalk.cyan("已经创建完毕"));
          setNodeEnv();
        }
      });
    } else {
...
export function setNvmMirror() {
  ...
  console.log(chalk.cyan("设置nvm镜像成功"));
}
...
export async function removeConfigs() {
  if (fs.existsSync(confPath)) {
    ...
    console.log(chalk.cyan("重置成功!"));
  } else {
    console.log(chalk.red("您还没有任何配置!"));
  }
}
