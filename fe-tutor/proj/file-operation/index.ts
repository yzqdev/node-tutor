...
export function readDir(dir: fs.PathLike) {
...
// 判断制定路径是否是文件
export function isFile(dir: fs.PathLike) {
  return fs.statSync(dir).isFile();
}

// 获取目录名
export function getDirName(dir: string) {
  let tempdir = dir.substring(dir.lastIndexOf("/") + 1, dir.length);
  return tempdir;
}
 ...
 */
export function getDirTree(dir: fs.PathLike) {
  let obj = {
    dir: dir, // 文件夹路径
    childFiles: [], // 子文件
    childDir: {}, // 子目录
