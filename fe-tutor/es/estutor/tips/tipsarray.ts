...
{
  console.log(`___________4.选择 Map / Object 字面量，而不是Switch语句_________________`);
  //bad
  ...
  //best
  // 使用 Map ，根据颜色找出对应的水果
  const fruitColorMap = new Map().set(`red`, [`apple`, `strawberry`]).set(`yellow`, [`banana`, `pineapple`]).set(`purple`, [`grape`, `plum`]);

  function testapple3(color) {
    return fruitColorMap.get(color) || [];
  }
  console.log(`我们是不是应该禁止使用 switch 语句呢？ 不要局限于此。 就个人而言，我尽可能使用对象字面量，但我不会设置硬规则来阻止使用 switch ，是否使用应该根据你的场景而决定`);
  const fruits = [
